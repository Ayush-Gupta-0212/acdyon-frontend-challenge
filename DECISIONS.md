...
contest reliably in progress, the panel would sometimes be empty. So it's simulated, fictional
handles, labelled as such. Honest and always-on beat real and occasionally creepy.
...
Everything measurable here was measured **against the deployed site**, not localhost — Lighthouse
97/100/100/100 desktop, **86**/100/100/100 mobile, WCAG AA on all 199 text elements, zero
horizontal overflow at eight widths. I'm quoting the mobile 86 rather than the 99 localhost gives
me, because I know exactly what it is: the hero entrance animation fades the `<h1>` — the LCP
element — from `opacity: 0`, so 2.6 s of the 3.2 s LCP is my own motion design. A real trade,
knowingly made, with a known fix (README).
